...
Принято для bootstrap `320-ai-control` и как базовая модель для будущего `410-ai-control`.
...
Каноническая Proxmox API identity управляющего AI-контура:
...
`ai-agent` обозначает владельца прав — AI-контур, а `infra` — конкретный токен инфраструктурного доступа. Identity не привязана к Hermes или Proximo. Сейчас токен использует общий Proximo MCP; замена MCP сама по себе не требует переименования identity, если назначение и права остаются прежними.

Права выдаются через этот отдельный Proxmox API user/token с privilege separation и ACL на выделенный resource pool управляемых гостей. Не использовать `root@pam`, `PVEAdmin` или ACL на весь `/` без отдельного решения.

Конкретный агент не должен получать отдельный более мощный Proxmox token только потому, что ему не хватает функции в MCP. Дополнительный token создаётся только когда действительно нужно разделить права, потребителей или возможность независимого отзыва credentials. Расширение прав выполняется отдельным инфраструктурным решением.
...
Планируемая write-зона обычных управляемых гостей:
...
`410-ai-control`, `320-ai-control`, production `100` и template `9000` не входят в обычную self-managed write-zone. Фактическое помещение любого гостя в `managed` выполняется только тогда, когда управляющему контуру действительно разрешено им управлять.
...
Целевой control plane остаётся вне собственной write-зоны AI control. Агенты, работающие внутри `410`, не должны иметь возможность остановить, удалить или перенастроить VM, в которой они сами работают. Изменения жизненного цикла `410` выполняются отдельным bootstrap/recovery механизмом или человеком.
...
После перехода на `410-ai-control` права на бывший bootstrap `320` определяются отдельным решением перед выводом из эксплуатации.
...
Полные права внутри конкретных управляемых Linux-гостей могут предоставляться через административного пользователя `ops` и `sudo`, но это не даёт AI agents дополнительных прав на PVE host.

## Главный принцип

> AI control agents администрируют выделенные гости, но не администрируют гипервизор, собственный control-plane guest, систему прав Proxmox или базовую инфраструктуру PVE host.
