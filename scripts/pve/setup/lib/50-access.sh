...
ensure_role() {
    ...
    if [[ -n "$extra" ]]; then
        dangerous_extra="$(printf '%s\n' "$extra" | grep -E '^(Permissions\.Modify|Sys\.Modify|Sys\.PowerMgmt|User\.Modify)}
...
warn_unexpected_permission_set() {
    ...
    local actual allowed extra

    actual="$(jq -r --arg path "$path" '(.[$path] // {}) | keys[]?' <<<"$permissions_json" 2>/dev/null | LC_ALL=C sort -u)"
    allowed="$(priv_lines "$allowed_raw")"
    extra="$(comm -13 <(printf '%s\n' "$allowed") <(printf '%s\n' "$actual") || true)"
    if [[ -n "$extra" ]]; then
        AI_PERMISSION_BOUNDARY_WARNINGS=1
        warn "API-токен ${full_token} имеет дополнительные effective permissions на ${path}: $(join_lines "$extra"). Они не отзываются автоматически."
    fi
}
...
warn_forbidden_permission_set() {
    ...
    while IFS= read -r priv; do
        ...
        if jq -e --arg path "$path" --arg priv "$priv" '((.[$path] // {}) | has($priv))' <<<"$permissions_json" >/dev/null 2>&1; then
            found+="${priv}"    local userid=$1 token_name=$2 full_token=$3 scope=$4 permissions_json
...
ensure_pve_identities() {
 ...
 || true)"
        ...
        if [[ -n "$dangerous_extra" ]]; then
            warn "Роль ${role} содержит опасные дополнительные privileges, которые PVE Configuration не удаляет автоматически: $(join_lines "$dangerous_extra"). Требуется отдельная проверка и осознанное сужение прав."
        else
            warn "Роль ${role} содержит дополнительные privileges: $(join_lines "$extra"). Они сохранены без изменений, но модель минимальных прав считается отклонённой."
        fi
    fi
}
...
\n'
    ...
    done < <(priv_lines "$forbidden_raw")

    if [[ -n "$found" ]]; then
        AI_PERMISSION_BOUNDARY_WARNINGS=1
        warn "API-токен ${full_token} имеет запрещённые для своей области effective permissions на ${path}: $(join_lines "${found%    local userid=$1 token_name=$2 full_token=$3 scope=$4 permissions_json
...
\n'}"). Права не отзываются автоматически."
    fi
}
...
warn_unexpected_ai_acl_entries() {
    ...
    for principal_type in user token; do
        ...
        rows="$(jq -r --arg type "$principal_type" --arg ugid "$principal" \
            '.[] | select(.type == $type and .ugid == $ugid) | [.path, .roleid] | @tsv' \
            <<<"$acl_json" 2>/dev/null || true)"
        while IFS=    local userid=$1 token_name=$2 full_token=$3 scope=$4 permissions_json
