...
template_description_values() {
    local config=$1 key=$2 description segment
    description="$(sed -n 's/^description: //p' <<<"$config" | head -n1)"
...
template_description_field_equals() {
    local config=$1 key=$2 expected=$3 values count
    values="$(template_description_values "$config" "$key")"
    count="$(printf '%s\n' "$values" | sed '/^$/d' | wc -l)"
    [[ "$count" == "1" && "$values" == "$expected" ]]
}
...
check_template_state() {
    ...
    if [[ "$current_name" == "$TEMPLATE_BUILDER_NAME" ]] \
        ...
        || { [[ "$current_name" == "$TEMPLATE_NAME" ]] \
            && template_description_field_equals "$config" "template-version" "$TEMPLATE_VERSION" \
