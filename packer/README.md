...
Текущий базовый шаблон `9000` имеет `Template-Version 8`. Сборка запускается через `scripts/infra-manager/build-template.sh 9000`. Старый контур `templates/debian13` сохранён только временно как справочный материал до первой успешной реальной Packer-сборки и больше не является источником новой архитектуры.
