...
`http/preseed.cfg` автоматизирует установку Debian. Packer временно отдаёт его установщику по HTTP непосредственно из `infra-runtime`; файл не является частью готового шаблона.
...
Текущий базовый шаблон `9000` имеет `Template-Version 8`. Сборка запускается через `scripts/infra-manager/build-template.sh 9000`. Старый PVE-side контур удалён; Packer является единственным способом сборки шаблона.
