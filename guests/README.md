...
Специальный гость, жизненным циклом которого владеет не обычный гостевой контур, может использовать отдельное машинное описание вместо `guest.yaml`.
...
`910 infra-deployer` создаётся публичным bootstrap и поэтому намеренно не включается в обычное OpenTofu-состояние гостей.
