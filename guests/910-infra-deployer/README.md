...
Публичный `bootstrap-pve.sh` создаёт виртуальный объект 910, минимально подготавливает Debian, передаёт постоянный GitHub Deploy Key, получает закрытый проект и запускает его сценарии. Docker, Semaphore и остальные рабочие компоненты 910 настраиваются уже закрытым проектом.
...
- OpenTofu state;
- PVE API credential;
- копия GitHub Deploy Key для чтения закрытого проекта;
...
Постоянный GitHub Deploy Key создаётся публичным bootstrap и хранится на PVE:
...
Bootstrap передаёт копию ключа внутрь 910. Ключ используется только для чтения:

```text
git@github.com:zsergeyru/proxmox.git
```

При первом запуске public bootstrap показывает открытый ключ. Его нужно один раз добавить в GitHub как read-only Deploy Key.

При удалении и повторном создании 910 используется тот же ключ с PVE, поэтому повторная регистрация в GitHub не требуется.
