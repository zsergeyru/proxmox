...
| `compose/docker-compose.yml` | фактическую схему контейнеров Semaphore Server и Semaphore Runner |
| `compose/runner/` | состав образа Runner |
...
└─ public bootstrap
   ...
   └─ запускает scripts/infra-deployer/setup.sh
      ...
      ├─ запускает Semaphore Server
      ├─ собирает и запускает Runner
...
В самом Debian находятся только базовые средства, необходимые для получения проекта, работы Docker и обслуживания 910. OpenTofu, Ansible и Packer не устанавливаются как отдельные системные службы Debian: они входят в образ Semaphore Runner.
...
В первой версии используется SQLite.

Веб-интерфейс публикуется на порту:
...
```text
infra-deployer-runner
```

Runner выполняет инфраструктурные задания Semaphore.
...
OpenTofu state и другие постоянные данные не должны зависеть от жизненного цикла контейнера Runner.
...
`infra-deployer-status` проверяет локальное состояние 910, Docker, Semaphore, Runner, инструменты и базовую авторизацию PVE API.
...
- пересобирает Runner при необходимости;
...
Runner использует сеть 910 напрямую только затем, чтобы временный HTTP-сервер Packer был доступен Debian Installer. Отдельный веб-сервис для Packer не создаётся.
...
При первом создании `setup.sh` показывает в терминале:
...
В обычный журнал bootstrap пароль не записывается. При повторных запусках пароль не меняется и повторно не выводится.
