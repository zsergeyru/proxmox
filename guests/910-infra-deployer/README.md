...
Изменяющие права 910 распространяются на путь Proxmox `/vms`, то есть на все VM/LXC. Дополнительный ACL на `/pool/managed` сохраняется только для создания и назначения гостей в этот pool. Для сборки шаблонов Packer получает отдельный доступ к ISO-хранилищу `local`, а диски VM размещает через `local-lvm`. Сам `managed` остаётся границей для AI Control и не ограничивает infra-deployer.
