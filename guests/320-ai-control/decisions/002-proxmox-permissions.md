...
`301-ai-control`, `320-ai-control`, production `100` и template `9000` не входят в обычную self-managed write-zone. Фактическое помещение любого гостя в `managed` выполняется только тогда, когда управляющему контуру действительно разрешено им управлять.
...
Целевой control plane остаётся вне собственной write-зоны AI control. Агенты, работающие внутри `301`, не должны иметь возможность остановить, удалить или перенастроить VM, в которой они сами работают. Изменения жизненного цикла `301` выполняются отдельным bootstrap/recovery механизмом или человеком.
