...
Поэтому права на Proxmox не привязываются архитектурно к Hermes как к единственному агенту. Они принадлежат отдельной management identity управляющего контура и могут использоваться теми агентами, которым разрешён доступ к общему Proxmox MCP.
...
Точная команда `pveum` должна проверяться по фактической версии PVE и требованиям выбранного Proxmox MCP. Базово custom role должна покрывать guest lifecycle и не включать host administration.
