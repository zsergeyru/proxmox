...
Сейчас PVE использует Keenetic как DNS. Общая DNS-архитектура: [`../../../docs/dns.md`](../../../docs/dns.md).
...
`101-network-gateway` разворачивается в этой же LAN по адресу `192.168.1.32/16`. Keenetic остаётся edge-router и default gateway, поэтому остановка 101 не должна отключать обычный интернет.
...
`101-network-gateway` отвечает за SmartDNS, VPN/PBR, remote-access VPN и дополнительные сетевые сервисы.

Подробности и адресация находятся в [`../../../docs/network.md`](../../../docs/network.md).
...
PVE не должен быть маршрутизатором квартиры. Перезапуск PVE/VM 101 не должен отключать базовые DHCP, WAN и межсегментную маршрутизацию, которыми владеет физический edge-router.
