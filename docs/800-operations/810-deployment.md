...
До переноса архитектуры в `main` остаётся выполнить на реальном PVE явный тест полного жизненного цикла временного гостя. Статическая проверка и read-only проверка прав через API не заменяют этот тест.
