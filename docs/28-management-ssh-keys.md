...
Канонический каталог открытых административных ключей является постоянным изменяемым состоянием deploy-контура:
...
и изменяется непосредственно `deploy-guest`/`sync-management-keys` от имени `pvedeploy`. Отдельный root-helper или другой привилегированный посредник для регистрации открытых management keys не используется.

Это сознательная граница: `pvedeploy` уже владеет deployer private key и имеет штатный административный доступ `root` к управляемым Debian-гостям. Поэтому root-only registry открытых ключей не создавал бы отдельной существенной границы безопасности, но добавлял бы лишний привилегированный механизм. Root-trust сохраняется там, где он действительно нужен: для исполняемой доверенной копии проекта и root-only секретов PVE.
...
PVE Configuration обязана подготовить каталог и зарегистрировать в нём `deployer.pub`, но **не знает заранее**, какие будущие VM/LXC будут AI-, Ansible- или иными управляющими системами. Состав дополнительных ключей определяется только при развёртывании конкретных гостей.

Имя дополнительного файла зависит только от VMID. Имя гостя в имени `.pub` не используется: переименование VM/LXC не должно менять management identity.
...
→ если management.ssh_identity == true:
     ...
     зарегистрировать .pub на PVE как <VMID>.pub
...
Распространение открытых ключей выполняет **отдельная PVE-команда**, а не `deploy-guest` как основную функцию:
...
- наличие tag является необходимым, но не достаточным условием записи: тип объекта, ожидаемый Debian management SSH-контракт, ожидаемый адрес, сохранённый SSH host key и возможность аутентификации всё равно проверяются;
...
### 7.1. Первое запоминание SSH-ключа сервера

Проект работает в закрытой доверенной домашней сети. Для только что созданного нового гостя допускается простая модель первого знакомства:

```text
известен ожидаемый VMID и вычисленный адрес
→ дождаться SSH на этом адресе
→ получить SSH host key сервера
→ сохранить его в /var/lib/pvedeploy/.ssh/known_hosts
→ все дальнейшие подключения выполнять только со строгой проверкой сохранённого ключа
```

Для уже известного гостя неожиданная смена SSH host key означает `STOP`. Старая запись не удаляется автоматически только ради продолжения работы.

Основная практическая цель этой проверки в домашней сети — не допустить случайного подключения к другой машине после переиспользования адреса или изменения объекта.
...
Управляющий гость не регистрирует public keys на PVE и не получает право записи в файловую систему PVE.
...
- все дополнительные имена соответствуют формату `<VMID>.pub`;
...
## 12. Жизненный цикл, ротация и удаление

Обычный deploy и обычная синхронизация не ротируют private keys.

Для management identity действует простой жизненный цикл:
...
новый гость с тем же VMID  → создать новую identity; старую пару/старый .pub не переиспользовать
ротация                    → только отдельным явным действием
```

Если удаление VM/LXC выполняется механизмом, который не изменяет PVE registry напрямую, операция считается полностью завершённой только после удаления соответствующего `<VMID>.pub` и успешного `sync-management-keys`.

Повторное использование VMID не должно молча наследовать старую identity. Если перед созданием новой management identity обнаружен оставшийся `<VMID>.pub` от удалённого объекта, это состояние сначала должно быть явно очищено/подтверждено как часть удаления или переиспользования VMID.
...
→ только после этого окончательно считать старую identity отозванной
```
...
подготовить /var/lib/proxmox-deployer/public-keys как pvedeploy:pvedeploy
зарегистрировать deployer.pub
установить/подготовить sync-management-keys
```

Текущий PVE-хост вводится как новый и не имеет старого рабочего management-контура, который требовал бы миграции deployer SSH key. Поэтому отдельный механизм переноса старого `pve_guest_ed25519` сейчас не нужен.

Правило первого создания простое:

```text
если корректной пары deployer ещё нет → создать её по текущей схеме
если корректная пара уже есть → сохранить её
обычный повторный запуск → никогда не заменять её новым ключом автоматически
```
...
→ создаёт pvedeploy-owned public-key registry
...
→ забирает только .pub
→ регистрирует .pub как <VMID>.pub непосредственно в каноническом каталоге
→ после изменения каталога запускает sync-management-keys

sync-management-keys (pvedeploy)
→ рассматривает только PVE-объекты с tag management-ssh
→ для нового объекта один раз запоминает SSH host key ожидаемого адреса
→ для известного объекта строго сверяет сохранённый SSH host key
...
Это намеренно узкая модель. Универсальный secret broker, отдельный привилегированный helper для public-key registry, каталог закрытых ключей, VMID allowlist для публичных ключей, Git write-доступ ради регистрации ключей и SSH-доступ AI к PVE в неё не входят.
...
4. каталога `/var/lib/proxmox-deployer/public-keys/` с владельцем `pvedeploy:pvedeploy` в PVE Configuration;
5. первоначальной регистрации `deployer.pub` без отдельной legacy-миграции на текущем новом хосте;
6. обработки `management.ssh_identity` в `deploy-guest` с именем `<VMID>.pub`;
7. автоматического PVE tag `management-ssh` для deployer-created Debian-гостей;
8. требования ставить `management-ssh` при создании Debian-гостей AI/другим управляющим контуром;
9. отдельного `sync-management-keys`, использующего tag как область обнаружения;
10. первоначального запоминания SSH host key нового гостя в доверенной домашней сети и строгой проверки при дальнейших подключениях;
11. стандартных каталогов `/etc/proxmox-guest/ssh/` и `/etc/proxmox-guest/public-keys/`;
12. использования локального `management-authorized-keys` AI-контуром при создании новых Debian VM/LXC;
13. lifecycle-правил удаления `<VMID>.pub`, переиспользования VMID и явной ротации.
