...
## 8. Bootstrap SSH к 910

Для связи временного 990 с 910 используется отдельная постоянная bootstrap-идентичность, ограниченная только 910.

Закрытый ключ хранится на PVE:

~~~text
/var/lib/proxmox-bootstrap/ssh/910-bootstrap-ed25519
~~~

При создании 990 helper передаёт копию ключа только в:

~~~text
/var/lib/bootstrap-runner/ssh/guest_ed25519
~~~

OpenTofu устанавливает соответствующий public key в 910 при первоначальном создании LXC.

Этот ключ:

- не используется для остальных гостей;
- не является аварийным человеческим ключом;
- не передаётся AI-агентам;
- нужен только для bootstrap/recovery 910;
- хранится на PVE, который и без SSH является корнем доверия для своего LXC через pct.

Стабильная идентичность нужна для повторных запусков 990: новый временный контейнер не должен каждый раз менять ключ и тем самым создавать изменение initialization в OpenTofu.

## 9. Последовательность закрытого runner

Runner выполняется в две фазы.

Первая:

~~~text
run.sh deploy
→ проверить окружение 990
→ установить/проверить Docker
→ собрать infra-runtime
→ принять bootstrap-state 910
→ построить effective state только для VMID 910
→ OpenTofu plan/apply
→ Ansible применяет базовое provision 910
~~~

После этого public bootstrap выполняет на PVE существующий закрытый helper постоянного доступа 910. Он передаёт 910 PVE CA и постоянный API credential.

Вторая:

~~~text
run.sh configure
→ передать в 910 выбранную Git-ревизию
→ передать read-only GitHub Deploy Key
→ запустить scripts/infra-manager/setup.sh внутри 910
→ infra-manager-status --full
~~~

После успешной второй фазы helper 990 сохраняет bootstrap-state на PVE, отзывает временный token и только затем public bootstrap удаляет 990.
...
Если OpenTofu state ресурса 910 имеет статус `tainted`, общий deploy-guest для VMID 910 останавливается с ошибкой и не использует обычное автоматическое удаление повреждённого гостя. Состояние 990 и фактический LXC сохраняются для диагностики и явного восстановления.
