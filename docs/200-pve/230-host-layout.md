...
PVE Configuration никогда не удаляет `repo/` только потому, что он не распознан как корректная Git-копия. Если путь занят непустым неизвестным каталогом, файлом, симлинком или Git worktree с `.git`-файлом, конфигурация останавливается и оставляет данные без изменений.
...
`state.json` является снимком результата последнего запуска PVE Configuration. `last-revision` отдельно показывает текущую принятую ревизию канонической Git-копии и может быть новее после штатного запуска `deploy-guest`. Для проверки текущего фактического состояния используется `pve-configuration-status --check`.
