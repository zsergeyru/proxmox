...
3. Развернуть `101-network-gateway` в текущей LAN по адресу `192.168.1.32`: SmartDNS, исходящие VPN, PBR, remote-access VPN и health-check, сохранив обычный интернет через Keenetic независимо от 101.
4. Проверить будущую MAIN `10.0.0.0/16` ещё до переезда и постепенно выдавать новым серверам постоянные `10.0.X.YZ`.
...
8. Определить startup/shutdown order гостей. Аварийный доступ к PVE не должен зависеть от 101, 301 или 311.
...
10. Перед переездом внедрить OpenWrt как edge-router: WAN, VLAN termination, DHCP и базовый L3/firewall оставить на физическом роутере; DNS/VPN/PBR оставить на 101.
...
- после переезда OpenWrt отвечает за WAN/VLAN/DHCP/L3/firewall, а 101 — за DNS/VPN/PBR/remote-access;
