...
> **Переходное состояние:** текущие defaults, schemas и runtime-скрипты ещё содержат `management.ssh.user` и `management.ssh.port`. Они будут удалены из машинного контракта вместе с общей реализацией согласованных изменений.

В индивидуальном `guest.yaml` раздел `management` нужен только тогда, когда гостю требуется одна из дополнительных возможностей:
...
Если возможность не нужна, соответствующее поле не указывается.

Явное `false` в исходном `guest.yaml` не требуется: отсутствие поля означает, что возможность не запрошена.

В effective state resolver может нормализовать отсутствующие значения в `false`, чтобы машинное представление оставалось полным и детерминированным.

#### 3.5.1. `management.ssh_identity`
...
Если оно присутствует, значение должно быть `true`.

Наличие `management.ssh_identity` требует `profile`, потому что создание, регистрация и проверка identity относятся к управляемому deploy-контуру.

Если поле отсутствует, исходящий management SSH identity этому гостю не требуется.

#### 3.5.2. `management.project_repo_read`

Поле также задаётся только конкретному гостю.
...
`true` означает, что гостю нужен управляемый read-only доступ к приватному репозиторию проекта.

Поле не содержит имя ключа, путь к private key, URL репозитория или произвольные Git credentials. Оно только включает заранее определённую проектную возможность.

Если оно присутствует, значение должно быть `true`.

Наличие `management.project_repo_read` требует `profile`.

Если поле отсутствует, проектный read-only Git-доступ этому гостю не требуется.

`management.project_repo_read` не устанавливает Git client. Наличие Git как программы определяется отдельно через Bootstrap.
...
Дополнительные management-возможности:
...
задаются только в конкретном `guest.yaml` и только значением `true`.

Если поле отсутствует, соответствующая возможность не требуется.

В effective state отсутствующее значение может быть нормализовано в `false`.

Обе возможности требуют `profile` и применяются только после запуска гостя и появления проверенного административного SSH.

`management.ssh_identity` означает собственную исходящую SSH identity гостя.

`management.project_repo_read` означает фиксированный read-only доступ к репозиторию проекта и не заменяет установку Git client.
