...
Если bridge отсутствует в итоговом effective state, effective schema выдаёт ошибку.
...
Если он корректен, все обычные гости с `profile` получают его в effective state.
...
Если задано `true`, гость должен иметь `profile`, а `boot.start_after_deploy` должен быть `true`, потому что создание/проверка identity происходит после появления административного SSH.
