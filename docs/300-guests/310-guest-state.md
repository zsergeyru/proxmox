...
Для гостя с `profile` поле обязательно непосредственно в исходном `guest.yaml`.
...
Если поле отсутствует у гостя с `profile`, source schema выдаёт ошибку ещё до построения effective state.
...
Если поле отсутствует, source schema гостя с `profile` выдаёт ошибку.
...
Но если LXC с `profile` после объединения всё равно не имеет `swap_mb`, effective schema выдаёт ошибку.
...
Если `storage` отсутствует во всех источниках, effective state гостя с `profile` не проходит проверку.
