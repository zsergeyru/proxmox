...
- текущий статус реализации принятых контрактов;
...
| Узнать, что из принятых контрактов уже реализовано в коде | [`29-implementation-status.md`](29-implementation-status.md) |
...
| [`29-implementation-status.md`](29-implementation-status.md) | текущая степень реализации уже принятых контрактов |
...
Общепроектный [`29-implementation-status.md`](29-implementation-status.md) — исключение по назначению: это не план задач, а краткий справочник фактической степени реализации **уже принятых** контрактов. В нём нет сроков, приоритетов или новых требований.

### План задач

Отдельный `90-roadmap.md` не ведётся. Незавершённая работа не должна смешиваться с действующими контрактами. Реализованное устойчивое решение переносится в Политику, Спецификацию или ADR; фактическое состояние реализации уже принятого контракта фиксируется в `29-implementation-status.md`, а состояние конкретного объекта — в его `STATUS.md`.
...
`Статус` в этом блоке относится к жизненному циклу **самого документа**, а не к проценту готовности кода:

```text
Действующий
→ документ является актуальным нормативным источником; описанный runtime при этом может быть реализован полностью или частично

Черновик
→ контракт или изложение ещё не приняты как окончательные

Исторический
→ документ больше не задаёт текущее требуемое состояние
```

Текущая степень реализации действующих спецификаций отдельно фиксируется в [`29-implementation-status.md`](29-implementation-status.md). Поэтому `Статус: Действующий` у спецификации `deploy-guest` означает «это принятый контракт для реализации», а не «скрипт уже существует и полностью реализован».
