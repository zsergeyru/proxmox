...
До переезда VLAN не используются. Keenetic имеет отдельный WAN и обычную домашнюю LAN. Компьютеры и Proxmox подключены к разным физическим LAN-портам Keenetic, но находятся в одном L2-сегменте и одной подсети. `101-network-gateway` уже на этом этапе разворачивает DNS, VPN, PBR и остальные сетевые сервисы в той же LAN.
